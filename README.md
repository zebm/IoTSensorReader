...
Overall testing had missed a number of components (Configuration has no tests, and the Messaging Components have limited coverage, namely the EventHub consumer - mainly due to an external dependancy IoT Hub).

## Unit Testing
Unit tests verify individual components in isolation using mocks for dependencies. Each test focuses on a single expected behavior or edge case.

Tests are organized by component type:
- `Formatting.UnitTests/` - Tests for formatters (temperature, humidity, unknown, JSON)
- `Messaging.UnitTests/` - Tests for EventHub consumer, deserializer, and coordinator
- `Output.UnitTests/` - Tests for console, database, and composite output services
- `Processing.UnitTests/` - Tests for message processor and routing logic
- `Sensors.UnitTests/` - Tests for sensor reading handlers

Unit Testing Coverage:
![picture](readmepictures/unittestcoverage.png)



## Integration Testing
Integration tests verify that multiple components work together correctly with real implementations (not mocks). These tests use actual formatters, handlers, and processors.

**Tests are organized by workflow:**
- MessageProcessing/ - Tests full pipeline: consumer → deserializer → processor → output
- OutputServices/ - Tests composite and single output services working together and with MongoDB
- CompleteWorkflowIntegrationTest.cs - Full workflow from message reception to output

Unit Testing Coverage:
![picture](readmepictures/intergration_test_coverage.png)


## End-to-End (E2E) Testing
This methodlogy of testing allows for verification of the entrie end to end flow from IoT Device to Output.
This utilises a real IoT Hub and MongoDB.
